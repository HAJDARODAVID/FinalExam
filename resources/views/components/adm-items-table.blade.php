    ...
    <tbody>
        @foreach ($items as $item)
            <tr @if (!Route::has($item->route)) class="table-danger" @endif >
                ...
                <td>
                    @if ($item->route == 'admItemsModule')
                        <a class="btn btn-secondary" style = "width: 120px" href="#">{{ __('NO ACTION') }}</a>
                    @else
                        @if ($item->active == 1)
                            ...
                            <form 
                                ...
                                method="POST" 
                                action="{{ route('changeAdmItemStatus', ['id' => $item->id, 'type' => 'deactivate']) }}"  
                                class="d-none">
                                    @method('PUT')
                                    @csrf
                            </form>    
                        @else
                            ...
                            <form 
                                ...
                                method="POST" 
                                action="{{ route('changeAdmItemStatus', ['id' => $item->id, 'type' => 'activate']) }}"  
                                class="d-none">
                                    @method('PUT')
                                    @csrf
                            </form>   
                    ...
                    <button 
                        onclick="
                            ...
                            document.getElementById('modal-menu-item-form').action = '{{ route('editAdmMenuItem', $item->id) }}'"
                    ...
                    <form 
                      ...
                      method="POST" 
                      action="{{ route('deleteAdmMenuItem', $item->id) }}"  
                      class="d-none">
                        @method('DELETE')
                        @csrf
                    </form>
                    ...
                    <form 
                        id="up-{{ $item->id }}-form" 
                        method="POST" 
                        action="{{ route('changeAdmItemStatus', ['id' => $item->id, 'type' => 'up']) }}"  
                        class="d-none">
                            @method('PUT')
                            @csrf
                    </form>
                    ...
                    <form 
                        ...
                        method="POST" 
                        action="{{ route('changeAdmItemStatus', ['id' => $item->id, 'type' => 'down']) }}"  
                        class="d-none">
                            @method('PUT')
                            @csrf
                    </form>
